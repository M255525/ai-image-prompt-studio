...
- **主題拆解模式**（2026-09-14 新增，`f_decompose` 勾選框，位於「概念描述」欄位下方）：勾選後展開 `#decomposePanel`，按「🧩 拆解並發展概念」用**單一 LLM 呼叫**（不逐一分開呼叫每個次要主題，省 API 次數）請 AI 依固定格式依序完成「拆解→發展→整合」——把使用者填的概念描述拆解成 3～5 個次要主題、分別發展每個主題的具體內容、最後整合成一段更完整的繁體中文概念描述；`decomposePrompt()` 要求 AI 用固定文字標記（`【次要主題拆解與發展】`／`【整合後的完整概念描述】`）分段輸出，`parseDecomposeResult()` 純字串切割解析（不是 JSON，避免 AI 回傳格式不穩定的 JSON 解析失敗風險）。拆解結果（次要主題清單＋整合文字）先顯示在 `#decomposeResult` 供使用者確認，按「套用整合結果到概念描述」才會真的覆寫 `state.fields.concept`（既有內容非空時先 `confirm()` 詢問，行為比照「從商品網址抓取重點」）——**這個功能只是讓概念描述本身變得更豐富，不會跳過後續「組成提示詞」／「送給 AI 優化」步驟**。共用同一組 `#aiProvider`/`#aiModel`/`#aiKey` 連線設定與 `callLLM()`，不需要另外填金鑰；`decompose:false` 已併入 `defaultFields()`／`state.fields`，勾選狀態會隨其他欄位一起存進 `localStorage`（key 仍是 `imgPromptState`）與「已儲存的提示詞」，但只是 UI 開關本身，不影響組裝或 AI 優化的 prompt 內容。已用 Playwright 對 `callLLM` 的 `fetch()` 打樁模擬完整流程（勾選展開→呼叫→解析顯示→套用覆蓋確認）驗證通過。
