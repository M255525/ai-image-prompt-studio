...
- **主題拆解模式**（2026-09-14 新增；`f_decompose` 勾選框，位於「概念描述」欄位下方）：勾選後展開 `#decomposePanel`，按「🧩 拆解並發展概念」用單一 LLM 呼叫（省 API 次數）請 AI 依固定格式完成「拆解→發展→整合」三步驟——把概念拆解成 3～5 個次要主題、分別發展每個主題更豐富的衍生內容（2～3 句）、整合成一段更完整的繁體中文概念描述，**只負責豐富概念本身，完全不處理風格／構圖／光線／長寬比／負面提示詞**（那些交給下面「組成提示詞」／「送給 AI 優化」處理，職責刻意分開）。`decomposePrompt(concept)` 要求 AI 用固定文字標記（`DECOMPOSE_BREAKDOWN_MARKER='【次要主題拆解與發展】'`／`DECOMPOSE_INTEGRATED_MARKER='【整合後的完整概念描述】'`）分段輸出，`parseDecomposeResult()` 用容錯 regex（`DECOMPOSE_INTEGRATED_RE`／`DECOMPOSE_BREAKDOWN_RE`，括號可有可無、可加空格與 `*`/`#`/`：` 等修飾符號——實測 LLM 不會逐字重現標記，精確字串比對常找不到）解析，且**保底絕不因解析失敗而拋錯**：找不到整合標記時，直接把整段回應當成 `integrated`（次要主題拆解框則不顯示）。次要主題拆解顯示在 `#decomposeResult` 供使用者了解發想過程；整合後的概念**直接寫回 `state.fields.concept` 並更新 `#f_concept`**（原本非空、且套用後內容會改變時才用原生 `confirm()` 詢問是否覆蓋，行為比照「從商品網址抓取重點」）。套用後不需要任何額外操作，接著按「🔧 組成提示詞」或「🚀 送給 AI 優化」就會自動把這段更豐富的概念跟其他設定組合起來——這是刻意的簡化：**曾經一度把主題拆解直接接到 AI 優化、跳過組成提示詞這一步（見下方 `aiPromptForTarget()` 的「草稿翻譯」設計），但使用者實測認為應該回到「豐富概念→照舊走組成/優化兩步驟」的簡單分工，已於 2026-09-15 改回**。共用同一組 `#aiProvider`/`#aiModel`/`#aiKey` 連線設定與 `callLLM()`；`decompose:false` 已併入 `defaultFields()`／`state.fields`，隨其他欄位一起存進 `localStorage`（key `imgPromptState`），只是 UI 開關本身、不影響組裝或 AI 優化的 prompt 內容。已用 Playwright 驗證完整流程（勾選展開→呼叫→解析顯示次要主題→整合結果寫回 `#f_concept`→後續組成/優化自動帶入）。
- **`aiPromptForTarget()` 的「翻譯既有草稿」設計**（2026-09-15，取代原本「AI 自己從欄位重新組裝」的做法）：使用者實測回報「AI 優化結果」的 English Prompt 常常漏掉已勾選的風格標籤（例如 close-up shot、professional photography），且完全沒把「不想出現的元素」翻成任何英文負面描述——根因是原本讓 AI 直接從一串「指定風格：A、B、C」「不想出現的元素：X、Y、Z」的**列表**自己重新組織成 1～3 句英文描述，句數限制一擠壓，AI 很自然會選擇性省略「看起來不影響主要畫面」的項目。改法：不再讓 AI 自己記得列出每一項設定，而是先呼叫 `assembleCoreLines(target, v)`（`assembleForTarget()` 拆出的共用核心，不含給人類看的頁尾註記）取得「🔧 組成提示詞」那份**規則式組裝、100% 完整**的草稿（含 `【概念】`/`【風格標籤（英文）】`/`【長寬比】`或 Midjourney 參數/`【不想出現的元素】`），連同「請把這份草稿翻譯潤飾、每一項都要有對應內容、不可遺漏」的指示一起送給 AI——AI 的任務從「記得列出每一項」降低成「把已經列好的草稿翻成通順英文」，大幅降低遺漏風險。`hasExisting`（既有照片提示詞補充）分支不受影響，仍走原本「以既有提示詞為基礎補充」的邏輯，標籤／長寬比／負面元素照舊用條列句子提供上下文（因為那個情境沒有對應的「組成提示詞」草稿可以直接翻譯）。非 Midjourney 的 `formatRule` 句數也放寬到「2～4 句」給 AI 足夠空間。
  - **追加的保底機制（同日）**：光靠「請不要省略」仍是軟性要求——自然語言句子裡沒有像 Midjourney `--ar`/`--no` 那樣的強制參數語法可以依附。新增共用函式 `mandatoryEnglishTrailerRule(v, ratioOpt, neg)`，要求 English Prompt 結尾**必須**另外加上固定格式短句：`"Aspect ratio: 1:1, square format ideal for social media posts."` 與 `"Avoid: cluttered background, visible human hands, watermarks, text, logos."`，把長寬比與負面元素變成獨立於描述句之外、不需要 AI 自行判斷要不要犧牲的必填項目，效果比照 Midjourney 分支本來就有的 `--ar`/`--no` 強制參數。`aiPromptForTarget()` 的非 Midjourney `formatRule` 都串接這個共用函式。已用 Playwright 截取實際送給 `callLLM()` 的完整 prompt 文字，確認完整流程（決解豐富概念 → 組成提示詞草稿 → AI 優化 prompt）都正確帶入 `【長寬比】`/`【不想出現的元素】` 與 `Aspect ratio:`/`Avoid:` 強制指示。
  - **中英兩段完整度不對等的踩坑（2026-09-15 同日，使用者實測回報「英文提示詞沒有問題，但中文並沒有改」）**：上面的保底機制只加強了 English Prompt，「中文說明」原本的規則從頭到尾都只是「簡短描述你理解的畫面內容，方便使用者確認語意」——這個措辭本來就是設計成一句話摘要，不要求涵蓋長寬比／不想出現的元素，所以即使 English Prompt 已經完整，中文說明仍然停留在簡短風格，兩段的完整度明顯不對等，使用者感覺「中文沒有改」。修法：新增 `mandatoryChineseTrailerRule(v, ratioOpt, neg)`，要求中文說明結尾也**必須**另外加上固定格式短句——「長寬比：1:1，1:1 方形（社群貼文）。」與「避免：雜亂背景、浮水印／文字／Logo、人手入鏡。」，跟 `mandatoryEnglishTrailerRule()` 的英文版本對稱；**這條規則對所有目標模型分頁（含 Midjourney）都套用**，因為「中文說明」這個輸出區塊本身是四個分頁共用的固定格式，不像 English Prompt 只有 Midjourney 才有 `--ar`/`--no` 可以依附。「輸出規則」第 1 條的措辭也從「簡短描述你理解的畫面內容」改成「完整度必須跟 English Prompt 對等，不是一句話摘要，要把風格／構圖／光線／色調也具體寫進去」；結尾的完整度總結句也從「English Prompt 必須完整涵蓋」改成「中文說明與 English Prompt 都必須完整涵蓋」。已用 Playwright 截取實際送出的 prompt，確認「中文說明的最後也要另外加一句固定寫法：「長寬比：...」」與「「避免：...」」兩條指示都正確出現在送給 AI 的請求內容中。
