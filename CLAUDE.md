...
- **主題拆解模式**（2026-09-14 新增，2026-09-15 改為直接輸出到 AI 優化結果；`f_decompose` 勾選框，位於「概念描述」欄位下方）：勾選後展開 `#decomposePanel`，按「🧩 拆解、發展並優化提示詞」用**單一 LLM 呼叫**（不逐一分開呼叫每個次要主題、也不需要再另外呼叫一次「送給 AI 優化」，省 API 次數）請 AI 依固定格式依序完成「拆解→發展→整合→優化」四步驟——把使用者填的概念描述拆解成 3～5 個次要主題、分別發展每個主題更豐富的衍生內容（2～3 句，非最初版本的 1～2 句）、跟目前 `state.fields` 的風格／構圖／光線／色調／長寬比／負面提示詞（`buildTagsZh()`／`buildNegativeList()`／`optFind(RATIO_OPTIONS,...)`，與 `aiPromptForTarget()` 同一套取值）整合，最後直接依**目前分頁**（`state.activeTarget`）的格式規則（Midjourney 逗號關鍵詞＋`--ar --v`／其餘自然語言句子，`formatRule` 邏輯照抄 `aiPromptForTarget()`）產出中英對照提示詞。`decomposeAndOptimizePrompt(target, v)` 要求 AI 用固定文字標記（`DECOMPOSE_BREAKDOWN_MARKER='【次要主題拆解與發展】'`／`DECOMPOSE_RESULT_MARKER='【AI優化結果】'`）分段輸出，`parseDecomposeOptimizeResult()` 純字串切割解析（不是 JSON，避免 AI 回傳格式不穩定的解析失敗風險）。次要主題拆解內容只顯示在 `#decomposeResult` 供使用者了解 AI 的發想過程（**不再需要「套用」按鈕**）；`【AI優化結果】` 標記之後的內容（中文說明＋`---`＋English Prompt，格式與 `aiPromptForTarget()` 產出的結構完全一致）**直接寫入 `state.aiOutput[state.activeTarget]` 並渲染進 `#aiOutput`**，跟按「🚀 送給 AI 優化」是同一個輸出區與同一顆「複製 AI 結果」按鈕，**會直接覆蓋目前分頁既有的 AI 優化結果、不另外詢問確認**（比照 `aiGenerateBtn` 本身也是直接覆蓋、無 confirm 的既有行為）。`#decomposeTargetLabel`（面板提示文字內的目標模型簡稱）由 `renderTargetPanel()` 同步更新，切分頁時會跟著變。共用同一組 `#aiProvider`/`#aiModel`/`#aiKey` 連線設定與 `callLLM()`，不需要另外填金鑰；`decompose:false` 已併入 `defaultFields()`／`state.fields`，勾選狀態會隨其他欄位一起存進 `localStorage`（key 仍是 `imgPromptState`）與「已儲存的提示詞」，但只是 UI 開關本身，不影響組裝或 AI 優化的 prompt 內容。已用 Playwright 對 `callLLM` 的 `fetch()` 打樁模擬完整流程（勾選展開→呼叫→解析顯示次要主題→結果寫入並渲染 `#aiOutput`）驗證通過。
  - **已修的踩坑（2026-09-15，使用者實測回報「AI 優化結果沒有出現預期內容」）**：最初 `parseDecomposeOptimizeResult()` 用 `text.indexOf('【AI優化結果】')` 精確字串比對兩個標記，但實測發現 LLM 不一定逐字重現這個標記——常見變形包括括號內多一個空格（`【 AI優化結果 】`）、用 markdown 粗體／標題符號包住（`**【AI優化結果】**`）、甚至整段不用全形括號。精確比對找不到就直接 `throw new Error('AI 回應格式異常...')`，導致 `#aiOutput` 完全沒被寫入、卡在錯誤訊息，使用者看到的就是「並沒有出現如同 hint 所述的內容」。修法：`DECOMPOSE_RESULT_RE`／`DECOMPOSE_BREAKDOWN_RE` 改用容錯 regex（括號可有可無、可加空格與 `*`/`#`/`：` 等修飾符號），且**保底絕不 throw**——連兩個標記都完全找不到時，直接把整段 AI 回應原文當成 `optimized` 寫入 `#aiOutput`（只是不再額外顯示 `#decomposeResult` 次要主題拆解框），確保只要 API 呼叫成功，「AI 優化結果」欄位一定有內容可看。已用 Playwright 對三種變形回應（空格＋粗體括號／無括號純文字標題／完全沒有任何標記）逐一驗證都能正確寫入 `#aiOutput`。
  - **內容完整度踩坑（2026-09-15，使用者實測回報 AI 優化結果「比原有規格少很多」）**：使用者比對「🔧 組成提示詞」的純關鍵字組裝結果（含全部指定的風格／構圖／光線標籤與不想出現的元素）跟「AI 優化結果」的 English Prompt，發現後者明顯漏掉多個已指定的標籤（例如漏了 close-up shot、professional photography），且完全沒有把「不想出現的元素」（雜亂背景、人手入鏡、浮水印／文字／Logo）翻成任何英文負面描述。根因是 `formatRule`（`aiPromptForTarget()` 與 `decomposeAndOptimizePrompt()` 共用同一套文字）原本只要求「1～3 句流暢英文描述句」、只把「不想出現的元素」列成參考資訊，沒有明確要求 AI 必須逐項涵蓋——句數限制加上沒有強制要求，AI 為了寫出精簡通順的句子，很自然會選擇性省略部分標籤與全部負面描述。修法：兩個函式的非 Midjourney `formatRule` 句數放寬為「2～4 句」；標籤說明與不想出現的元素說明文字都加註「不可為了精簡而省略任何一項」／「也要逐項用明確的英文負面描述表達（例如 no watermark or logo, hands out of frame, uncluttered background）」；`decomposeAndOptimizePrompt()` 的步驟三整合指示也加註「不可在整合過程中遺漏」；兩者都在「English Prompt 輸出規則」後另外加一行明確要求「必須完整涵蓋每一項標籤與不想出現的元素，完整度優先於簡潔」。Midjourney 分支的 `--no` negative 處理措辭也從「視情況加上」改成「完整列出每一項」。已用 Playwright 截取實際送給 `callLLM()` 的完整 prompt 文字，確認新增的完整度要求確實出現在送出的請求內容中。**這是 prompt 措辭層面的加強、不是解析邏輯的 bug**——AI 仍可能因為模型本身的取捨而省略內容，只是機率會明顯降低；無法 100% 保證，實測若仍有遺漏，可請使用者換更聽指令的模型，或縮減同時勾選的標籤數量。
  - **後續追加（2026-09-15 同日，使用者再次回報長寬比與不想出現的元素仍然消失）**：上一輪只把「不可省略」寫進說明文字裡，本質仍是**軟性要求**——自然語言句子裡沒有像 Midjourney `--ar`/`--no` 那樣的強制參數語法可以依附，長寬比與負面元素能不能出現，完全取決於 AI 自己記不記得把它們寫進句子。這類「請不要省略」的提醒在句數限制（2～4 句）的擠壓下，仍然很容易被 AI 判斷為「不影響主要畫面描述、可以犧牲」的次要資訊。修法：新增共用函式 `mandatoryEnglishTrailerRule(v, ratioOpt, neg)`，把長寬比與負面元素從「融入主句的軟性建議」改成「English Prompt 結尾必須另外加上的固定格式短句」——`"Aspect ratio: 1:1, square format ideal for social media posts."` 與 `"Avoid: cluttered background, visible human hands, watermarks, text, logos."`，讓這兩項變成獨立於描述句之外、不需要 AI 自己判斷「要不要犧牲」的必填項目，效果類似 Midjourney 分支本來就有的 `--ar`/`--no` 強制參數。`aiPromptForTarget()`／`decomposeAndOptimizePrompt()` 的非 Midjourney `formatRule` 都串接這個共用函式（`ratioOpt` 因此提前到函式開頭計算，兩處原本各自重複宣告的 `var ratioOpt` 已合併成一次）。已用 Playwright 截取實際送出的 prompt，確認一般「送給 AI 優化」與「主題拆解優化」兩條路徑在有勾選長寬比／負面標籤時，prompt 內容都確實包含 `Aspect ratio:` 與 `Avoid:` 這兩條強制指示。
